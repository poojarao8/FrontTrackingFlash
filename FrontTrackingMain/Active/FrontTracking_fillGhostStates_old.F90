        ...
        do jcell = blkLimitsGC(LOW,JAXIS), blkLimitsGC(HIGH,JAXIS)
            do kcell = blkLimitsGC(LOW,KAXIS), blkLimitsGC(HIGH,KAXIS)
                ...
                if (c == 1) then 
                    ...
                    do i = icell - 1, icell + 1     
                        do j = jcell - 1, jcell + 1
                            do k = kcell - 1, kcell + 1
                                ...
                                if ((i .lt. icell) .or. &
                                    (i .eq. icell .and. j .lt. jcell) .or. &
                                    (i .eq. icell .and. j .eq. jcell .and. k .lt. kcell)) then
                                    if (solnData(G1DENS_VAR,i,j,k) .gt. 0.0) then
                                ...
                                ! JAM : This was designed to restrict it from
                                ! filling in a ghost state if none of the
                                ! immediate neighbors were of that component,
                                ! but this causes some problems, so testing with
                                ! this restriction not required
                                ! A more sophisticated check such as if the cell
                                ! is within a "stencil length" of the other
                                ! component as is implemented in cFluid may be
                                ! necessary
                                !if (solnData(COMP_VAR,i,j,k) .eq. 2) then
                                    counter = counter + 1
                                !end if
                    ...
                    if (counter .gt. 0 .and. num .gt. 0) then
