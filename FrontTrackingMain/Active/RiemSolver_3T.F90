...
! is U_L.  if ustar < 0, then the state on the axis is U_R.
     ...
     if(ustar .eq. 0.0) then
        regIndic = '  |  '
     else if(ustar .lt. 0.0) then
        regIndic = '<----'
     else if(ustar .gt. 0.0) then
        regIndic = '---->'
     end if
     ...
     if (pstar - ps .gt. 0.e0) then ! shock ?
        regIndic(3:3) = 'S'
     else if (pstar - ps .lt. 0.e0) then
        regIndic(3:3) = 'R'
     end if
...
! compute some factors, Fry Eq. 81 and 82       
     ...
     if (westar .lt. 0.0 .and. wes .gt. 0.0) then
        regIndic(2:2) = '|'
        regIndic(4:4) = '|'
     end if
  ...
  do n = 0, HYDRO_NUM_EINT_COMPONENTS
        eintAv(n) = hy_eiLft(n) * scrch2 + hy_eiRght(n) * scrch3
  enddo
  do n = 0, HYDRO_NUM_E_COMPONENTS
        etotAv(n) = hy_eLft(n) * scrch2 + hy_eRght(n) * scrch3
  enddo
  do n = 1, HYDRO_NUM_E_COMPONENTS
        pFracAv(n) = hy_pFracLft(n) * scrch2 + hy_pFracRght(n) * scrch3
  enddo
  do n = 1, HYDRO_NUM_GAME_COMPONENTS
        gameav(n) = hy_gmeLft(n) * scrch2 + hy_gmeRgt(n) * scrch3
  enddo
...
! compute correct state for rarefaction fan by linear interpolation
    ...
    !JAM - Put solutions into midstate vector
    midstate(DENS_VAR) = rhoav
    midstate(PRES_VAR) = pav
    midstate(VELX_VAR) = uav
    midstate(GAME_VAR) = gameav
